...
Create an editable environment with `pip install -e .` and include test helpers with `pip install -e .[test]`. Run the demo locally with `python demo.py --help` or a concrete command such as `python demo.py status`. Run the test suite with `python -m unittest discover -s ./tests -b`. For packaged-install checks, use `python -m pip install .` in a clean virtualenv.
